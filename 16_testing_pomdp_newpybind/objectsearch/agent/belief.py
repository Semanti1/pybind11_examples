...
class MosOOBelief(OOBelief):
    ...
    def __init__(self, robot_id, object_beliefs):
        ...
        OOBelief.__init__(self,object_beliefs)

    def mpe(self, **kwargs):
        return MosOOState(pomdp_py.OOBelief.mpe(self, **kwargs).object_states)

    def random(self, **kwargs):
        return MosOOState(OOBelief.random(self, **kwargs).object_states)
...
def _initialize_histogram_belief(dim, robot_id, object_ids, prior, robot_orientations):
    ...
    oo_hists[robot_id] =\
        Histogram({RobotState(robot_id, init_robot_pose, (), None): 1.0})
